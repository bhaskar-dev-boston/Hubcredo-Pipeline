...
export default function Leads() {
  ...
  async function handleGenerateLeads() {
    ...
    setScraping(true);
    try {
      await triggerScraping.mutateAsync({ data: { user_id: me.id, icp_id: currentIcp.id, lead_list_id: selectedListId } });
      ...
      <div className="p-8 max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8 pt-2">
          <div>
            ...
            <p className="text-[#64748B] text-sm">Generate and manage qualified prospects</p>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            <p className="text-xs text-[#64748B] uppercase tracking-widest font-medium px-1 mb-3">Lead lists</p>
            {listsLoading ? (
              ...
              <div className="bg-white border border-dashed border-[#E2E8F0] rounded-xl p-6 text-center">
                ...
                <button onClick={() => setShowNewList(true)} className="text-xs text-[#2563EB] hover:text-[#1D4ED8] mt-1 transition-colors">
                  Create one
                </button>
              </div>
            ) : (
              (leadLists as LeadList[]).map((list) => (
                ...
                >
                  <p className="text-sm font-medium">{list.label || "Untitled list"}</p>
                  <p className={`text-xs mt-0.5 capitalize ${selectedListId === list.id ? "text-[#2563EB]/70" : "text-[#64748B]"}`}>{list.status}</p>
                </button>
              ))
            )}
          ...
          <div className="lg:col-span-2 space-y-5">
            {selectedList && (
              <div className="bg-white border border-[#E2E8F0] rounded-xl p-5 space-y-4 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
                <div className="flex items-center justify-between">
                  <h3 className="text-[#0A0A0A] font-semibold text-sm">Generate leads for "{selectedList.label || "list"}"</h3>
                ...
                {currentIcp ? (
                  ...
                  <p className="text-xs text-[#DC2626]">No ICP set. Go to Settings to define your target audience.</p>
                )}
                <button
            ...
            {!selectedListId ? (
              <div className="bg-white border border-dashed border-[#E2E8F0] rounded-xl p-12 text-center">
                ...
                <p className="text-[#0A0A0A] font-medium mb-1">Select a lead list</p>
                <p className="text-sm text-[#64748B]">Choose a list on the left to view and generate leads</p>
              </div>
            ) : leadsLoading ? (
              ...
              <div className="bg-white border border-dashed border-[#E2E8F0] rounded-xl p-12 text-center">
                <p className="text-sm text-[#64748B]">No leads in this list yet. Generate some above.</p>
              </div>
            ) : (
              <div className="space-y-2">
                <p className="text-xs text-[#64748B] uppercase tracking-widest font-medium mb-3">{(leads as Lead[]).length} leads</p>
                {(leads as Lead[]).map((lead) => {
                  const displayName = [lead.first_name, lead.last_name].filter(Boolean).join(" ") || "Unknown";
                  return (
                    <div
                      key={lead.id}
                      className="bg-white border border-[#E2E8F0] rounded-xl p-4 flex items-start gap-4 hover:border-[#CBD5E1] hover:bg-[#F5F7FA] transition-colors"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[#64748B] mt-0.5">{lead.job_title}{lead.company_name ? ` · ${lead.company_name}` : ""}</p>
